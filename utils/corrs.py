...
def build_am_corrs(mah_df_dict: dict[pd.DataFrame],
                   df0: pd.DataFrame) -> list:
    corrs_list = []
    filtered_m = []
    params_dict = {}
    masses = datutils.unique_masses(mah_df_dict)
    for m in masses:
        mah_df = datutils.am_mbin(m, mah_df_dict)
        if len(mah_df) < 200:
            continue
    ...
    return params_dict, corrs_list, filtered_m
